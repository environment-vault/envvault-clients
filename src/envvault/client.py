...
class EnvVaultClient:
    ...
    def _request(self, method: str, path: str, **kwargs: Any) -> Any:
        ...
        try:
            try:
                ...
                raise EnvVaultError(f"Request timed out after {self.timeout}s") from None
            except (ConnectionError, OSError):
                raise EnvVaultError(f"Cannot connect to EnvVault server at {self.server_url}") from None
        finally:
            conn.close()
    ...
    def render_secrets_dotenv(
        ...
        """
        Build .env-style text from key-value secrets (same rules as ``export_dotenv``).

        Does not write to disk. Suitable for stdout or piping.
        """
        secrets = self.get_secrets(project_id, environment, version_name=version_name)
        version_label = f" | Version: {version_name}" if version_name else ""
        lines = [
            "# Generated by EnvVault SDK",
    ...
    def export_dotenv(
        ...
        body = self.render_secrets_dotenv(project_id, environment, version_name=version_name)
